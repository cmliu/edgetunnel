...
| **SUB_TOTAL_GB** | ❌ | `100` | 订阅总流量（GiB），用于 `Subscription-Userinfo` 响应头，默认 `100` |
| **SUB_EXPIRE** | ❌ | `4102329600` | 订阅到期时间（Unix 秒级时间戳），默认为 2099-12-31 |

> [!NOTE]
> 流量统计会复用 `KV` 绑定，按 UUID 保存在 `traffic:<UUID>` 键中；客户端刷新订阅时会通过 `Subscription-Userinfo` 读取。为降低 KV 写入次数，流量会在连接内批量刷新，并发连接较多时可能存在少量误差，不建议用于严格计费。
